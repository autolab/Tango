...
class Preallocator:
    ...
    def poolSize(self, vmName):
        ...
        if vmName not in self.machines:
            ...
            return len(self.machines[vmName][0])
    ...
    def update(self, vm, num):
        ...
        if vm.name not in self.machines:
            self.machines[vm.name] = [[], Queue.Queue()]
            self.log.debug("Creating empty pool of %s instances" % (vm.name))
        self.lock.release()

        delta = num - len(self.machines[vm.name][0])
        if delta > 0:
    ...
    def freeVM(self, vm):
        ...
        not_found = False
        self.lock.acquire()
        if vm and vm.id in self.machines[vm.name][0]:
            self.machines[vm.name][1].put(vm)
        else:
            not_found = True;
        self.lock.release()
    ...
    def addVM(self, vm):
        ...
        """
        self.lock.acquire()
        self.machines[vm.name][0].append(vm.id)
        self.lock.release()
    ...
    def removeVM(self, vm):
        ...
        """
        self.lock.acquire()
        self.machines[vm.name][0].remove(vm.id)
        self.lock.release()
    ...
    def replaceVM(self, vm):
        ...
        self.lock.acquire()
        self.machines[vm.name][0].remove(vm.id)
        size = self.machines[vm.name][1].qsize()
        if (size == len(self.machines[vm.name][0])):
            for i in range(size):
                otherVM = self.machines[vm.name][1].get_nowait()
                if otherVM.id != vm.id:
                    self.machines[vmName][1].put(otherVM)
                else:
                    dieVM = otherVM;
        self.lock.release()
        ...
        else: #VM to be replaced not found.
            return -1
    ...
    def __destroy(self, vm):
        ...
        """
        self.lock.acquire()
        dieVM = self.machines[vm.name][1].get_nowait()
        self.lock.release()
    ...
    def destroyVM(self, vmName, id):
        ...
        dieVM = None
        self.lock.acquire()
        size = self.machines[vmName][1].qsize()
        if (size == len(self.machines[vmName][0])):
            for i in range(size):
                vm = self.machines[vmName][1].get_nowait()
                if vm.id != id:
                    self.machines[vmName][1].put(vm)
                else:
                    dieVM = vm;
        self.lock.release()
