...
10. Perform SSL setup:
	a. Ensure that your DNS record points towards the IP address of your server
	b. Ensure that port 443 is exposed on your server (i.e checking your firewall, AWS security group settings, etc)
	c.  Get initial SSL setup script: `make ssl`
	d. In `ssl/init-letsencrypt.sh`, change `domains=(example.com)` to the list of domains that your host is associated with, and change `email` to be your email address so that Let's Encrypt will be able to email you when your certificate is about to expire
	e. If necessary, change `staging=0` to `staging=1` to avoid being rate-limited by Let's Encrypt since there is a limit of 20 certificates/week. Setting this is helpful if you have an experimental setup.
	f. Stop your containers: `docker-compose stop`
	g. Run your modified script: `sudo sh ./ssl/init-letsencrypt.sh`
